...
void print_qr_terminal(const char *text) {
    ...
    for (int y = 0; y < qrcode->width; y++) {
        for (int x = 0; x < qrcode->width; x++) {
            // Usa blocos Unicode para formar a imagem 
...
void conectar_instancia() {
    // RESOLVIDO: Captura das variáveis de ambiente movida para dentro da função 
    char *api_url = getenv("EVOLUTION_URL") ? getenv("EVOLUTION_URL") : "http://evolution-reservatorio:8080";
    ...
    if(curl) {
        ...
        char auth[250]; 
        // RESOLVIDO: Ajustado para usar 'api_key' minúsculo de forma consistente 
        snprintf(auth, sizeof(auth), "apikey: %s", api_key);
        headers = curl_slist_append(headers, auth);

        char url[300];
        // RESOLVIDO: Ajustado para usar 'api_url' minúsculo de forma consistente 
        snprintf(url, sizeof(url), "%s/instance/connect/%s", api_url, instance);
